...
public interface productAttributeRepository extends JpaRepository<ProductAttribute, Long>{
    ...
    @Query(value = "select * from TBLPRODUCTATTRIBUTE "
            ...
            + "and CASE WHEN :PRODUCTCATEGORY_ID = 0 THEN PRODUCTCATEGORY_ID=PRODUCTCATEGORY_ID ELSE PRODUCTCATEGORY_ID IN (:PRODUCTCATEGORY_IDS) END "
            + "and CASE WHEN :PRODUCT_ID = 0 THEN PRODUCT_ID=PRODUCT_ID ELSE PRODUCT_ID IN (:PRODUCT_IDS) END "
            + "and CASE WHEN :ATTRIBUTECATEGORY_ID = 0 THEN ATTRIBUTECATEGORY_ID=ATTRIBUTECATEGORY_ID ELSE ATTRIBUTECATEGORY_ID IN (:ATTRIBUTECATEGORY_IDS) END "
			+ "and ISACTIVE='Y' order by ATTRIBUTEORDER_NO ", nativeQuery = true)
	List<ProductAttribute> findByAdvancedSearch(
		    @Param("ATTRIBUTE_ID") Long ATTRIBUTE_ID, @Param("ATTRIBUTE_IDS") List<Integer> ATTRIBUTE_IDS,
		    @Param("PRODUCTCATEGORY_IDS") Long PRODUCTCATEGORY_ID, @Param("PRODUCTCATEGORY_IDS") List<Integer> PRODUCTCATEGORY_IDS,
    		@Param("PRODUCT_IDS") Long PRODUCT_ID, @Param("PRODUCT_IDS") List<Integer> PRODUCT_IDS,
    		@Param("ATTRIBUTECATEGORY_IDS") Long ATTRIBUTECATEGORY_ID, @Param("ATTRIBUTECATEGORY_IDS") List<Integer> ATTRIBUTECATEGORY_IDS);
	...
	@Query(value = "select * from TBLPRODUCTATTRIBUTE "
			...
			+ "and CASE WHEN :ATTRIBUTECATEGORY_ID = 0 THEN ATTRIBUTECATEGORY_ID=ATTRIBUTECATEGORY_ID ELSE ATTRIBUTECATEGORY_ID IN (:ATTRIBUTECATEGORY_IDS) END "
			, nativeQuery = true)
	List<ProductAttribute> findAllByAdvancedSearch(
		    @Param("ATTRIBUTE_ID") Long ATTRIBUTE_ID, @Param("ATTRIBUTE_IDS") List<Integer> ATTRIBUTE_IDS,
		    @Param("PRODUCTCATEGORY_IDS") Long PRODUCTCATEGORY_ID, @Param("PRODUCTCATEGORY_IDS") List<Integer> PRODUCTCATEGORY_IDS,
    		@Param("PRODUCT_IDS") Long PRODUCT_ID, @Param("PRODUCT_IDS") List<Integer> PRODUCT_IDS,
    		@Param("ATTRIBUTECATEGORY_IDS") Long ATTRIBUTECATEGORY_ID, @Param("ATTRIBUTECATEGORY_IDS") List<Integer> ATTRIBUTECATEGORY_IDS);

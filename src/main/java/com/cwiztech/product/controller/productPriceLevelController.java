...
public class productPriceLevelController {
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public ResponseEntity insertupdateAll(JSONArray jsonProductPriceLevels, JSONObject jsonProductPriceLevel, JSONObject apiRequest) throws JsonProcessingException, JSONException, ParseException {
		...
		for (int a=0; a<jsonProductPriceLevels.length(); a++) {
			...
			if (jsonObj.has("productpricelevel_ID")) {
				...
				if (productpricelevelid != 0) {
					...
					if (productpricelevel == null)
						return new ResponseEntity(getAPIResponse(null, null , null, null, "Invalid ProductPriceLevel Data!", apiRequest, true), HttpStatus.BAD_REQUEST);
				}
			}
			...
			if (productpricelevelid == 0) {
				...
				if (!jsonObj.has("product_QUANTITY") || jsonObj.isNull("product_QUANTITY"))
					return new ResponseEntity(getAPIResponse(null, null , null, null, "product_QUANTITY is missing", apiRequest, true), HttpStatus.BAD_REQUEST);

				if (!jsonObj.has("product_UNITPRICE") || jsonObj.isNull("product_UNITPRICE"))
					return new ResponseEntity(getAPIResponse(null, null , null, null, "product_UNITPRICE is missing", apiRequest, true), HttpStatus.BAD_REQUEST);
			}
	...
	String getAPIResponse(List<ProductPriceLevel> productpricelevels, ProductPriceLevel productpricelevel , JSONArray jsonProductPriceLevels, JSONObject jsonProductPriceLevel, String message, JSONObject apiRequest, boolean isWithDetail) throws JSONException, JsonProcessingException, ParseException {

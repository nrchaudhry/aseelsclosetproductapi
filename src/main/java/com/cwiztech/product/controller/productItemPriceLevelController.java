...
public class productItemPriceLevelController {
		...
		for (int a=0; a<jsonProductItemPriceLevels.length(); a++) {
			...
			if (jsonObj.has("productitempricelevel_ID")) {
				...
				if (productitempricelevelid != 0) {
					...
					if (productitempricelevel == null)
						return new ResponseEntity(getAPIResponse(null, null , null, null, "Invalid ProductItemPriceLevel Data!", apiRequest, true), HttpStatus.BAD_REQUEST);
				}
			}
			...
			if (productitempricelevelid == 0) {
				...
				if (!jsonObj.has("productitem_UNITPRICE") || jsonObj.isNull("productitem_UNITPRICE"))
					return new ResponseEntity(getAPIResponse(null, null , null, null, "productitem_UNITPRICE is missing", apiRequest, true), HttpStatus.BAD_REQUEST);
			}
	...
	String getAPIResponse(List<ProductItemPriceLevel> productitempricelevels, ProductItemPriceLevel productitempricelevel , JSONArray jsonProductItemPriceLevels, JSONObject jsonProductItemPriceLevel, String message, JSONObject apiRequest, boolean isWithDetail) throws JSONException, JsonProcessingException, ParseException {
		...
		if (message != null) {
			...
			if (productitempricelevel != null && isWithDetail == true) {
				JSONObject productitem = new JSONObject(ServiceCall.GET("productitem/"+productitempricelevel.getPRODUCTITEM_ID(), apiRequest.getString("access_TOKEN"), false));
				productitempricelevel.setPRODUCTITEM_DETAIL(productitem.toString());

				if (productitempricelevel.getCURRENCY_ID() != null) {
					JSONObject currency = new JSONObject(ServiceCall.GET("lookup/"+productitempricelevel.getCURRENCY_ID(), apiRequest.getString("access_TOKEN"), true));
					productitempricelevel.setCURRENCY_DETAIL(currency.toString());
				}
				if (productitempricelevel.getPRICELEVEL_ID() != null) {
					JSONObject priceLevel = new JSONObject(ServiceCall.GET("lookup/"+productitempricelevel.getPRICELEVEL_ID(), apiRequest.getString("access_TOKEN"), true));
	...
	public ResponseEntity getAllByCustomer(@RequestBody String data, @RequestHeader(value = "Authorization") String headToken, @RequestHeader(value = "LimitGrant") String LimitGrant) throws JsonProcessingException, JSONException, ParseException {

...
private static final Logger log = LoggerFactory.getLogger(productItemPriceLevelController.class);
	...
	public ResponseEntity get(@RequestHeader(value = "Authorization") String headToken) throws JsonProcessingException, JSONException, ParseException {
		...
		return new ResponseEntity(getAPIResponse(productitempricelevels, null , null, null, null, apiRequest, false).getREQUEST_OUTPUT(), HttpStatus.OK);
	}
	...
	public ResponseEntity getOne(@PathVariable Long id, @RequestHeader(value = "Authorization") String headToken) throws JsonProcessingException, JSONException, ParseException {
		...
		return new ResponseEntity(getAPIResponse(null, productitempricelevel , null, null, null, apiRequest, false).getREQUEST_OUTPUT(), HttpStatus.OK);
	}
	...
	public ResponseEntity insertupdateAll(JSONArray jsonProductItemPriceLevels, JSONObject jsonProductItemPriceLevel, APIRequestDataLog apiRequest) throws JsonProcessingException, JSONException, ParseException {
		...
		for (int a=0; a<jsonProductItemPriceLevels.length(); a++) {
			...
			if (jsonObj.has("productitempricelevel_ID")) {
				...
				if (productitempricelevelid != 0) {
					...
					if (productitempricelevel == null)
						return new ResponseEntity(getAPIResponse(null, null , null, null, "Invalid ProductItemPriceLevel Data!", apiRequest, false).getREQUEST_OUTPUT(), HttpStatus.BAD_REQUEST);
				}
			}
			...
			if (productitempricelevelid == 0) {
				...
				if (!jsonObj.has("productitem_UNITPRICE") || jsonObj.isNull("productitem_UNITPRICE"))
					return new ResponseEntity(getAPIResponse(null, null , null, null, "productitem_UNITPRICE is missing", apiRequest, false).getREQUEST_OUTPUT(), HttpStatus.BAD_REQUEST);
			}
		...
		if (jsonProductItemPriceLevel != null)
			responseentity = new ResponseEntity(getAPIResponse(null, productitempricelevels.get(0) , null, null, null, apiRequest, true).getREQUEST_OUTPUT(), HttpStatus.OK);
		else
			responseentity = new ResponseEntity(getAPIResponse(productitempricelevels, null , null, null, null, apiRequest, true).getREQUEST_OUTPUT(), HttpStatus.OK);
	...
	public ResponseEntity delete(@PathVariable Long id,@RequestHeader(value = "Authorization") String headToken) throws JsonProcessingException, JSONException, ParseException {
		...
		return new ResponseEntity(getAPIResponse(null, productitempricelevel , null, null, null, apiRequest, true).getREQUEST_OUTPUT(), HttpStatus.OK);
	}
	...
	public ResponseEntity ByAdvancedSearch(String data, boolean active, String headToken) throws JsonProcessingException, JSONException, ParseException {
        ...
        if (jsonObj.has("product_ID") && !jsonObj.isNull("product_ID") && productitem_ID == 0 ) {
            ...
            for (int i=0; i<productsObject.length(); i++) {
                List<ProductItemPriceLevel> productitempricelevel = new ArrayList<ProductItemPriceLevel>();
                productitempricelevel = ((active == true)
                        ? productitempricelevelrepository.findByAdvancedSearch((long) 0, productsObject.getJSONObject(i).getLong("productitem_ID") , (long) 0 )
                        : productitempricelevelrepository.findAllByAdvancedSearch((long) 0, productsObject.getJSONObject(i).getLong("productitem_ID") , (long) 0 ));
                for (int j=0; j<productitempricelevel.size(); j++) {
                	productitempricelevels.add(productitempricelevel.get(j));
                }
            }
        }
        ...
        if(currency_ID != 0 || productitem_ID != 0 || pricelevel_ID != 0){
        	 List<ProductItemPriceLevel>  productitempricelevel = ((active == true)
		        ? productitempricelevelrepository.findByAdvancedSearch(currency_ID,productitem_ID,pricelevel_ID)
		        : productitempricelevelrepository.findAllByAdvancedSearch(currency_ID,productitem_ID,pricelevel_ID));
        for (int i=0; i<productitempricelevel.size(); i++) {
            boolean found = false;
            
            for (int j=0; j<productitempricelevels.size(); j++) {
                if (productitempricelevel.get(i).getPRODUCTITEMPRICELEVEL_ID() == productitempricelevels.get(j).getPRODUCTITEMPRICELEVEL_ID()) {
                    found = true;
                    break;
                }
            }
            ...
            if (found == false) {
            	productitempricelevels.add(productitempricelevel.get(i));
            }
        }
        }
		return new ResponseEntity(getAPIResponse(productitempricelevels, null , null, null, null, apiRequest, false).getREQUEST_OUTPUT(), HttpStatus.OK);
	...
	APIRequestDataLog getAPIResponse(List<ProductItemPriceLevel> productitempricelevels, ProductItemPriceLevel productitempricelevel , JSONArray jsonProductItemPriceLevels, JSONObject jsonProductItemPriceLevel, String message, APIRequestDataLog apiRequest, boolean isTableLog) throws JSONException, JsonProcessingException, ParseException {
		...
		if (message != null) {
			...
			} else if(productitempricelevels != null){
	...
	public ResponseEntity getAllByCustomer(@RequestBody String data,@RequestHeader(value = "Authorization") String headToken) throws JsonProcessingException, JSONException, ParseException {
		...
		return new ResponseEntity(getAPIResponse(null, null , objProductItems, null, null, apiRequest, false).getREQUEST_OUTPUT(), HttpStatus.OK);
	}
	...
	public ResponseEntity ChangePrice(@RequestBody String data, @RequestHeader(value = "Authorization") String headToken)
		...
		for (int i = 0; i < jsonPAV.length(); i++) {
				...
				List<ProductItemPriceLevel> productitempricelevel = productitempricelevelrepository.findByAdvancedSearch((long) 0, productitem_id, pricelevel_id);
